...
However, all samples were either divergent or reached max-treedepth.
This would likely not be solved with increasing the adaptive delta
the max-treedepth, since the output was quite catastrophic. The next
step would be to try a different parametrization with a logistic
normal distribution instead of a dirichlet, whic effectively
translates to @eq-logistic-softmax,

$$
logR ~ \mathcal{N}(\mu, \sigma)
...
that are easier interpreted and calibrated than the concentration
