...
delta = rng.lognormal(1, 1)
# we multiply by number of molecules per cell-volume to get number of molecules mRNA_i
Prot_per_cell_liter = 3E6
P = np.stack(
  [rng.dirichlet(R[cond, :] * Prot_per_cell_liter * delta)
...
phi_mrna_sim = np.stack(
  [rng.dirichlet(R[cond, :] * alpha_mrna * Prot_per_cell_liter)
  for cond in range(N_cond)
  ]
)
phi_prot_sim = np.stack(
  [rng.dirichlet(P[cond, :] * alpha_prot * Prot_per_cell_liter)
  for cond in range(N_cond)
  ]
)
```
...
#df_plot["prot"] = np.log(df_plot["prot"])
#df_plot["mrna"] = np.log(df_plot["mrna"])
